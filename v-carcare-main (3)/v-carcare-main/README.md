...
### ใช้งานจากมือถือใน Wi-Fi เดียวกัน

ตัวแอปรับการเชื่อมต่อจากอุปกรณ์อื่นในเครือข่ายอยู่แล้ว (bind ที่
`0.0.0.0`) ให้ทำตามนี้บนเครื่องที่รันเซิร์ฟเวอร์:

1. ให้มือถือและคอมพิวเตอร์เชื่อมต่อ Wi-Fi เดียวกัน และปิด VPN ที่อาจ
   แยกเครือข่ายออกจากกัน
2. ดู IPv4 ของคอมพิวเตอร์ด้วย PowerShell:

   ```powershell
   ipconfig
   ```

   มองหาบรรทัด `IPv4 Address` เช่น `192.168.1.25`
3. อนุญาตพอร์ต 5000 ผ่าน Windows Firewall (เปิด PowerShell แบบ Run as
   Administrator เพียงครั้งเดียว):

   ```powershell
   New-NetFirewallRule -DisplayName "V CarCare Flask" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 5000
   ```
4. รันแอปตามปกติ แล้วเปิดบนมือถือด้วย IP ของคอมพิวเตอร์ เช่น
   `http://192.168.1.25:5000` — ห้ามใช้ `localhost:5000` เพราะบนมือถือ
   `localhost` หมายถึงตัวมือถือเอง

> หมายเหตุ: ฟังก์ชันเปิดกล้อง/สแกนใบหน้าบนมือถือมักต้องใช้ HTTPS ตาม
> นโยบายของเบราว์เซอร์ แม้หน้าเว็บทั่วไปจะเปิดผ่าน HTTP ใน Wi-Fi ได้
> ตามปกติ
